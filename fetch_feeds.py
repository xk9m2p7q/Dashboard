...
def fetch_regional_indicators():
    ...
    # EUR/MAD cross rate (Morocco-Spain economic link)
    try:
        url = "https://v6.exchangerate-api.com/v6/8655aae699897e179ddbe554/latest/EUR"
        ...
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            mad = rates.get("MAD")
            if mad:
                indicators["eur_mad"] = {"value": round(float(mad), 2), "label": "EUR / MAD", "sub": "Morocco dirham", "date": date_today}
                print("EUR/MAD OK: " + str(mad))
    except Exception as e:
        print("EUR/MAD failed: " + str(e))

    # Venezuela parallel rate
    try:
        url = "https://s3.amazonaws.com/dolartoday/data.json"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=15) as r:
            data = json.loads(r.read().decode())
            parallel = data.get("USD", {}).get("transferencia", None)
            if parallel:
                indicators["vzla_parallel"] = {"value": round(float(parallel), 2), "label": "VES / USD", "sub": "Venezuela parallel", "date": date_today}
                print("VES parallel OK: " + str(parallel))
    except Exception as e:
        print("Venezuela parallel failed: " + str(e))

    # Spain 10Y bond yield via ECB
    try:
        url = "https://data.ecb.europa.eu/data/data/IRS/M.ES.L.L40.CI.0.EUR.N.Z/data?lastNObservations=1&format=csvdata"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            for line in reversed(lines):
                ...
                if len(parts) >= 2:
                    try:
                        val = float(parts[-1])
                        indicators["spain_10y"] = {"value": round(val, 3), "label": "Spain 10Y", "sub": "Bond yield %", "date": date_today}
