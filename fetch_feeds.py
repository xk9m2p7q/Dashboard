...
def fetch_exchange_rates():
    try:
        ...
        url = "https://api.frankfurter.app/latest?from=USD&to=EUR,GBP,MAD"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=15) as r:
            data = json.loads(r.read().decode())
            rates = data.get("rates", {})
            date = data.get("date", "")
            print("Rates OK: " + str(rates))
            return {
        ...
        print("Rates fetch failed: " + str(e))
        ...
        try:
            ...
            with urllib.request.urlopen(req2, timeout=15) as r2:
                ...
                date2 = data2.get("time_last_update_utc", "")[:10]
                print("Rates fallback OK")
                return {
                    "EUR": round(float(rates2.get("EUR", 0)), 4),
                    "GBP": round(float(rates2.get("GBP", 0)), 4),
                    "MAD": round(float(rates2.get("MAD", 0)), 2),
                    "date": date2
                }
...
EVENTS = [
    {"label": "German Federal Election", "date": "2025-09-28", "region": "europe"},
    {"label": "UN General Assembly", "date": "2025-09-16", "region": "global"},
    {"label": "COP30 (Belem)", "date": "2025-11-10", "region": "global"},
    {"label": "French Regional Elections", "date": "2025-06-15", "region": "europe"},
    {"label": "Venezuela: Maduro term review", "date": "2025-08-01", "region": "venezuela"},
    {"label": "Morocco: Budget deadline", "date": "2025-07-01", "region": "morocco"},
    {"label": "Iran: Nuclear talks deadline", "date": "2025-06-01", "region": "mena"},
    {"label": "EU Council Summit", "date": "2025-06-26", "region": "europe"},
    {"label": "NATO Summit", "date": "2025-06-24", "region": "global"},
    {"label": "Spain: Regional elections", "date": "2025-10-15", "region": "spain"},
]
