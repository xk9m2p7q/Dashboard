...
def fetch_regional_indicators():
    """Fetch geopolitically relevant regional indicators."""
    indicators = {}
    date_today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    API_KEY = "8655aae699897e179ddbe554"

    # EUR/MAD - use same exchangerate-api key, base EUR
    try:
    ...
    # Venezuela - use exchangerate-api for official VES rate
    try:
    ...
    # Spain 10Y - use FRED API (St Louis Fed, free no key for basic)
    try:
        url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=IRLTLT01ESM156N&vintage_date=" + date_today
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            for line in reversed(lines):
                ...
                if len(parts) == 2 and parts[1] not in ["", ".", "VALUE"]:
                    try:
                        val = float(parts[1])
                        indicators["spain_10y"] = {"value": round(val, 3), "label": "Spain 10Y", "sub": "Bond yield %", "date": parts[0]}
    ...
    # Morocco phosphate price proxy - World Bank commodity data
    try:
        url = "https://api.worldbank.org/v2/en/indicator/PPHOSPH.A?downloadformat=csv&mrv=1"
        # Use exchangerate-api for a Morocco-relevant indicator instead
        # USD/MAD already covered, so add EUR/USD for eurozone context
        url2 = "https://v6.exchangerate-api.com/v6/" + API_KEY + "/pair/EUR/USD"
        req2 = urllib.request.Request(url2, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req2, timeout=15) as r2:
            data2 = json.loads(r2.read().decode())
            rate2 = data2.get("conversion_rate")
            if rate2:
                indicators["eur_usd"] = {"value": round(float(rate2), 4), "label": "EUR / USD", "sub": "Eurozone rate", "date": date_today}
                print("EUR/USD OK: " + str(rate2))
    except Exception as e:
...
def fetch_flights(region="mena"):
    """Fetch live flight data from OpenSky for a region."""
    REGIONS = {
        "mena": {"lamin": 15, "lamax": 40, "lomin": 25, "lomax": 65},
        "europe": {"lamin": 36, "lamax": 72, "lomin": -12, "lomax": 35},
        "world": {"lamin": -60, "lamax": 72, "lomin": -130, "lomax": 160},
    }
    # Fetch MENA + Europe for the dashboard (most relevant regions)
    all_flights = []
    for reg_name, bounds in REGIONS.items():
        if reg_name == "world":
            continue  # skip world to stay within rate limits
        try:
